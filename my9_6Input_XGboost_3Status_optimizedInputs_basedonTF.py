...
# ===== ستون‌هایی که باید حذف شوند (اما در فایل ورودی باشند) =====
EXCLUDE_COLUMNS = [
...
class DataLoaderAndPreprocessor:
    ...
    def load_and_preprocess(self):
        ...
        # تعیین تایم فریم‌های مجاز برای مدل فعلی
        ...
        # فیلتر ستون‌هایی که باید استفاده بشن
        def is_allowed_col(col_name):
            # ستون هدف و Sood_zarar رو کنار بگذاریم
            if col_name in ['Sood_zarar', 'target']:
                return False
            # حذف ستون‌های EXCLUDE_COLUMNS
            if col_name in EXCLUDE_COLUMNS:
                return False
            # ستون‌هایی که حداقل یکی از allowed_timeframes تو نامش باشه، نگه داشته میشن
