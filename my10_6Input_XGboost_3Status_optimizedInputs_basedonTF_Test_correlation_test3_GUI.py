...
class ModelTrainer:
    ...
    def train(self, X, y):
        ...
        show_table(display_df, "Test Results - All Features + Sood_zarar")
...
class Predictor:
    ...
    def preprocess(self, row_dict):
        ...
        return df[self.feature_columns], df

    def convert_probs(self, probs):
        return sum(p * v for p, v in zip(probs, [-1, 1]))

    def predict(self, row_dict):
        X, transformed_df = self.preprocess(row_dict)
        proba = self.model.predict_proba(X)[0]
        pred = self.model.predict(X)[0]
        transformed_df = transformed_df[self.feature_columns]
        transformed_df['Predicted_Class'] = pred
        transformed_df['Profit_Proba'] = proba[1]
        transformed_df['Loss_Proba'] = proba[0]
        return pred, self.convert_probs(proba), {0: 'Loss', 1: 'Profit'}[pred], proba, transformed_df
            ...
            try:
                ...
                for row in all_inputs:
                    for tag, predictor in predictors.items():
                        pred_class, cont_pred, label, probs, transformed_df = predictor.predict(row)
                        ...
                        show_table(transformed_df, f"Prediction Result - {tag}")
