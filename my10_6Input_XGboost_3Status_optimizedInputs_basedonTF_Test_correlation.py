...
DATA_FOLDER_L =  'C:/pythonFiles/inputs/CSVBuy' # 'G:/3-ALL Python and AI/my codes/inputs'
...
# ===== Function to check and drop highly correlated features =====
def check_feature_correlation(X, threshold=0.9):
    corr_matrix = X.corr()  # Calculate correlation matrix
    correlated_features = set()
    for i in range(len(corr_matrix.columns)):
        for j in range(i):
            if abs(corr_matrix.iloc[i, j]) > threshold:  # Check for high correlation
...
class ModelTrainer:
    ...
    def train(self, X, y):
        ...
        # Feature correlation check and reduction
        X_train = check_feature_correlation(X_train)  # Remove correlated features
