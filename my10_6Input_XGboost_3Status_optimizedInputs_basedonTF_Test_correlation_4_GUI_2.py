...
def filter_and_categorize(df):
    df = df[~df.isin(["NotExist"]).any(axis=1)].copy()
...
class DataLoaderAndPreprocessor:
    ...
    def load_and_preprocess(self):
        ...
        self.print_numeric_feature_means()
        ...
        for col in selected_cols:
            if not is_numeric_column(features[col]):
    ...
    def print_numeric_feature_means(self):
        numeric_data = self.filtered_data.select_dtypes(include='number')
        means = numeric_data.mean()
        print(f"\n✅ میانگین متغیرهای عددی برای {self.model_tag} (بعد از فیلتر 'NotExist'):")
        print(means.round(4))
        print('-' * 50)
...
class ModelTrainer:
    ...
    def train(self, X, y):
        ...
        X_train = check_feature_correlation(X_train)
        self.selected_columns = list(X_train.columns)
...
class Predictor:
    ...
    def preprocess(self, row_dict):
        if any(str(v).strip() == "NotExist" for v in row_dict.values()):
            return None, True
        ...
        return df[self.feature_columns], False

    def convert_probs(self, probs):
        return sum(p * v for p, v in zip(probs, [-1, 1]))

    def predict(self, row_dict):
        X, invalid = self.preprocess(row_dict)
        if invalid:
            return -1, -1, "Invalid", [0.0, 0.0]
        ...
        df = X.copy()
        df["Prediction"] = pred
        df["Profit_Proba"] = proba[1]
        df["Loss_Proba"] = proba[0]

        root = tk.Tk()
        root.title("Prediction Output")
        frame = tk.Frame(root)
        frame.pack(fill='both', expand=True)
...
if __name__ == '__main__':
    ...
    for tag in MODEL_TAGS:
        ...
        selected_cols = trainer.get_selected_columns()
        predictor = Predictor(model, loader.label_encoders, loader.cat_columns, selected_cols)
